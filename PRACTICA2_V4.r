...
# TARGET RATING
hist(as.numeric(wine$rating))
ggtitle("Normal Q-Q Plot") 
qqnorm(as.numeric(wine$rating), pch = 19, col = "gray50")
qqline(as.numeric(wine$rating))
ks.test(x = as.numeric(wine$rating),"pnorm", mean(as.numeric(wine$rating)), sd(as.numeric(wine$rating)))
jarque.bera.test(x = as.numeric(wine$rating))
...
for (i in 1:(ncol(wine) - 2)) {
  spearman_test <- cor.test(wine[,i], as.numeric(y),method="spearman")
...
# elimino Births:Continent y ajusto+ANOVA
